...
public partial class DatabaseRepository
{
    public async Task<MemberSecurityRecord?> GetMemberSecurityRecordByEmailAsync(string email)
    {
        ...
        const string sql =
            "SELECT m.email Email, m.password HashedPassword, m.login_expiry LoginExpiry " +
        ...
        if (!string.Equals(member.Email, email, StringComparison.OrdinalIgnoreCase)) return null;
